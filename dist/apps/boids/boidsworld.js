...
export class BoidsWorld {
    constructor(size, boidsCount, flockCount = 1, center = new Vec3()) {
        this.boids = [];
        this.size = size;
        this.center = center;
        for (let i = 0; i < boidsCount; i++) {
            const flockId = Math.ceil(Math.random() * flockCount);
            const position = new Vec3(this.size.x * Math.random() - this.size.x / 2 + center.x, this.size.y * Math.random() - this.size.y / 2 + center.y, this.size.z * Math.random() - this.size.z / 2 + center.z);
            const velocity = new Vec3(Math.random(), Math.random(), Math.random()).normalize();
            this.boids.push(new Boid(this, flockId, position, velocity));
        }
    }
    getBoidsInFov(center, fov, maxAngle, sameFlock) {
