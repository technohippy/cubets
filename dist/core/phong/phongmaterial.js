...
export class PhongMaterial extends Material {
    constructor(diffuseColor = RGBAColor.random(), ambientColor = RGBAColor.Gray, specularColor = RGBAColor.White, shininess = 100) {
        super();
    ...
    setupGLVars(renderer, mesh) {
        ...
        if (ignoreTextureLocation)
            gl.uniform1i(ignoreTextureLocation, ignoreFlags.texture);
        if (ignoreNormalTextureLocation)
            gl.uniform1i(ignoreNormalTextureLocation, ignoreFlags.normal);
        if (ignoreCubeTextureLocation)
            gl.uniform1i(ignoreCubeTextureLocation, ignoreFlags.cube);
    }
}
