...
export class OBJGeometryLoader {
    static load(url) {
        const loader = new OBJGeometryLoader();
    ...
    load(url) {
        return fetch(url).then(resp => resp.json()).then((json) => {
            ...
            if (!json.vertices)
                throw 'invalid json: vertices is mandatory';
            if (!json.indices)
                throw 'invalid json: indices is mandatory';
            ...
            for (let i = 0; i < json.indices.length; i += 3) {
                geometry.indices.push(new Face3(json.indices[i], json.indices[i + 1], json.indices[i + 2]));
            }
            if (json.normals) {
            ...
            if (json.uvs) {
                for (let i = 0; i < json.uvs.length; i += 2) {
                    geometry.uvs.push(new Vec2(json.uvs[i], json.uvs[i + 1]));
                }
            }
            else {
                geometry.computeUvs();
            ...
            if (json.colors) {
                for (let i = 0; i < json.colors.length; i += 4) {
                    geometry.colors.push(new RGBAColor(json.colors[i], json.colors[i + 1], json.colors[i + 2], json.colors[i + 3]));
                }
            }
            return geometry;
        });
